...
const data = require('fs')
  .readFileSync(test ? 'input.test.txt' : `${__dirname}/input`, { encoding: 'utf-8' })
  .trim();

const groups = data.split(/\n{2,}/);

const algo = groups[0].split('').map((v) => v === '#');
...
for (let i = 0; i < 50; i++) {
  ...
  for (let x = -1 - i; x < width + i + 1; x++)
    for (let y = -1 - i; y < height + i + 1; y++)
      if (pixel(x, y, [0 - i, 0 - i, width + i - 1, height + i - 1], algo[0] ? i % 2 : 0))
        next.add(`${x},${y}`);
