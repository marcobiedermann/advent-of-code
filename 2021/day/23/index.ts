...
const inp = readFileSync(`${__dirname}/input`, 'utf-8');
...
const input = (raw) => {
  ...
  const hallway = [...Array(11)].map((_, i) => (i > 0 && i < 10 && i % 2 === 0 ? null : false));
...
const play = (state, init = false) => {
  ...
  for (let hallSpot = 0; hallSpot < hallway.length; hallSpot += 1) {
    ...
    if (rooms[toad].every((t) => t === toad || t === false)) {
      ...
      if (allClear) {
        ...
        const roomIndex = rooms[toad].filter((spot) => spot === false).length - 1;
        ...
        const newCost =
          (Math.abs(hallSpot - homeSpot) + rooms[toad].filter((t) => t === false).length) *
          10 ** toad;
  ...
  for (let room = 0; room < rooms.length; room += 1) {
    ...
    const tryHallSpot = (hallSpot) => {
      ...
      const newCost = (depth + 1 + Math.abs(toadSpot - hallSpot)) * 10 ** rooms[room][depth];
    ...
    for (let hallSpot = toadSpot + 1; hallSpot < hallway.length; hallSpot += 1) {
      if (hallway[hallSpot] === null) {
        // Unoccupiable
        continue;
      }
